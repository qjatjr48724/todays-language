...
| 해시 | 메시지 |
|------|--------|
| `aa053bf` | feat(mobile): 학습 리마인드 온보딩·로컬 알림·설정 재설정 |
| `46cb451` | docs: [단계 51] 학습 리마인드 온보딩·설정 재설정 진행 기록 |
...
| `b1ddd5b` | docs: [단계 52] 처리방침·Play 체크리스트 진행 기록 |
...
| `4892365` | docs: [단계 54] 회원 탈퇴·처리방침 진행 기록 |
...
## [단계 56] 탈퇴 데이터 완전 삭제·7일 재가입 차단 E2E·비밀번호 찾기 (2026-09-01)

### 1) 오늘 한 일

**탈퇴 Firestore 하위 컬렉션 삭제 보완 (Functions)**
- 증상: 탈퇴 후 `users/{uid}/daily_quiz_cursor` 등 cursor 컬렉션이 고아(orphan)로 남음
- 원인: Firestore는 부모 `users/{uid}` 삭제 시 하위 컬렉션을 자동 삭제하지 않음. 기존 `deleteUserFirestoreProfile`은 `daily_progress`만 삭제
- 수정: `USER_FIRESTORE_SUBCOLLECTIONS` — `daily_progress`, `daily_quiz_cursor`, `daily_word_cursor`, `daily_sentence_cursor` 전부 배치 삭제 후 부모 문서 삭제
- 테스트: `delete_user_data.test.ts`에 subcollection 계약 테스트 추가
- `deleteAccount` 재배포·검증(사용자)

**7일 재가입 차단 E2E (사용자 검증)**
- `before_user_created_block.ts`: `HttpsError` import를 `firebase-functions/v2/identity`로 수정 (blocking function 요구사항)
- 탈퇴 → 동일 이메일 재가입 시 앱 i18n **「탈퇴 후 7일 이내…」** 메시지 정상 표시 확인
- Firestore `account_recreation_blocks/{emailHash}` 기록·TTL(사용자 Console 설정) 연동은 [단계 55] 이전 커밋(`55ad585`) 기준

**비밀번호 찾기 (Flutter)**
- `password_reset_screen.dart` — `sendPasswordResetEmail()` + 성공 안내 UI
- `email_login_screen.dart` — 하단 `회원가입` 오른쪽에 **비밀번호 찾기** 링크
- i18n ko/en/ja, Analytics `password_reset` 화면 ID
- v1: 브라우저에서 재설정(딥링크 없음). 미가입 이메일도 성공 UI(Firebase 열거 방지)

**보류·결정**
- **이메일 인증(verify email)**: 모바일 UX(메일 앱 전환) 부담으로 **추가하지 않기로 결정**
- 향후 **휴대폰(SMS) 본인 확인**은 가입/탈퇴·민감 기능용으로 별도 검토

### 2) 합의·결정

- 탈퇴 시 사용자 Firestore 하위 경로는 **cursor 포함 전부** 삭제가 정본
- 비밀번호 찾기는 Firebase Auth 기본 재설정 메일(v1). Console Templates 커스터마이즈는 선택
- 소셜 로그인 추가 시 Google/Apple은 `emailVerified` 자동 true → password 가입자만 별도 인증 정책 검토(현재는 이메일 인증 미적용)

### 3) 완료 기준 체크

- [x] `deleteAccount` — cursor subcollection 포함 삭제 로직
- [x] `npm run test` (functions)
- [x] `deleteAccount` 재배포 (사용자)
- [x] 7일 재가입 차단 E2E — 커스텀 i18n 메시지
- [x] 비밀번호 찾기 화면·i18n·로그인 진입
- [x] `flutter gen-l10n` / `flutter analyze`
- [ ] `flutter test` widget_test Firebase 초기화 이슈(기존)
- [ ] `terms-ko.md` / `terms-en.md` 탈퇴·만 16세 문구 통일
- [ ] HTTPS URL · `lib/data/legal/` · Play/Apple 선언
...
| 영역 | 파일 |
|------|------|
| Functions | `delete_user_data.ts`, `delete_user_data.test.ts`, `before_user_created_block.ts` |
| Flutter | `password_reset_screen.dart`, `email_login_screen.dart`, `analytics_screens.dart`, `app_*.arb` |
| 문서 | `NOTION_PROGRESS_TEMPLATE.md` [단계 56] |

### 5) Git 커밋

| 해시 | 메시지 |
|------|--------|
| fd8a4c4 | fix: 탈퇴 시 daily cursor 하위 컬렉션 전체 삭제 |
| c5016c8 | feat: 이메일 비밀번호 찾기(재설정 메일) 화면 추가 |
| 4216411 | docs: [단계 56] 탈퇴 삭제 보완·재가입 차단 E2E·비밀번호 찾기 |
| 5cc4fa0 | docs: [단계 56] 중복 제거 및 커밋 해시 반영 |

### 6) 다음 액션

1. `main` push (로컬 `55ad585`~`4216411`)
2. 탈퇴 **이전** 계정의 고아 cursor — Firestore Console 수동 정리(필요 시)
...
| 00eb734 | docs: [단계 58] Git·Firebase 확인·법적 HTML 스타일 진행 기록 |
